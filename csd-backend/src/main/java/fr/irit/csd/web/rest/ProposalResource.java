...
import fr.irit.csd.repository.ProposalRepository;
...
/**
 * REST controller for managing {@link fr.irit.csd.domain.Proposal}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Proposal> partialUpdateProposal(
        ...
        Optional<Proposal> result = proposalRepository
            ...
            .map(existingProposal -> {
                ...
                if (proposal.getProposalState() != null) {
                    existingProposal.setProposalState(proposal.getProposalState());
                }
