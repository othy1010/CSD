...
export const resolvers = {
    Query: {
        ...
        collaboration(_, args) {
            return db.Collaborations.find(collaboration => collaboration.id === args.id);
        },
        users() {
            return db.Users;
        },
    ...
    Collaboration: {
        ...
        proposals(collaboration) {
            return db.Proposals.filter(proposal => collaboration.proposals.includes(proposal.id));
        }
    },
    User: {
        collaborations(user) {
            return db.Collaborations.filter(collaboration => user.collaborations.includes(collaboration.id));
        }
    },
    Proposal: {
        collaboration(proposal) {
            return db.Collaborations.find(collaboration => collaboration.id === proposal.collaboration);
        }
    },
    Mutation: {
        ...
        updateCollaboration(_, args) {
            ...
            return db.Collaborations.filter(collaboration => collaboration.id === args.id);
        },
        deleteCollaboration(_, args) {
            db.Collaborations = db.Collaborations.filter(collaboration => collaboration.id !== args.id);
            return db.Collaborations;
        },
        addUser(_, args) {
            const newUser = {
