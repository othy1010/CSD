...
import fr.irit.csd.repository.ParameterRepository;
...
/**
 * REST controller for managing {@link fr.irit.csd.domain.Parameter}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Parameter> partialUpdateParameter(
        ...
        Optional<Parameter> result = parameterRepository
            ...
            .map(existingParameter -> {
                if (parameter.getKind() != null) {
                    existingParameter.setKind(parameter.getKind());
                }
